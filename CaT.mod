...
Adapted from Anwar H, Hong S, De Schutter E (2010) 
Controlling Ca2+-activated K+ channels with models of Ca2+ buffering in Purkinje cell. Cerebellum*
...
PROCEDURE evaluate_fct(v(mV)) { 
        ...
        minf = 1.0 / ( 1 + exp((vhcatm-v)/5.499) )
        hinf = 1.0 / ( 1 + exp((v - vhcath)/10.22) )
        if (v<=-90) {
	taum = 1
	} else {
	taum = catfacm*(( C_tau_m + A_tau_m / (exp((v - v0_tau_m1)/ k_tau_m1) + exp((v - v0_tau_m2)/k_tau_m2))) / qt)
	}
	tauh = catfach*(( C_tau_h + A_tau_h / exp((v - v0_tau_h1)/k_tau_h1) ) / qt)
	g = ghk(v, cai, cao, 2)
}
